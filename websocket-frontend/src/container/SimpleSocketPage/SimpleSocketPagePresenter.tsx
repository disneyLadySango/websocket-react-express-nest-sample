    ...
    <StyledContents>
      <StyledRow>
        <Typography variant="body1">
          ...
          <ul>
            <li>
              APIリクエストをすると「REST API Hello
              「送信したメッセージ」!」が表示されます
            </li>
            <li>APIリクエストを契機に3回サーバから通知がプッシュされます</li>
            <li>
              コネクション接続・切断を押すと接続時は切断、切断時は再接続されます
            </li>
            <li>
              useEffectのreturnで切断処理を行っています。ページを離れると（ヘッダーの文字を押してもらえると・・・）コネクションが切れます
            </li>
...
const StyledDataContainer = styled.fieldset`
  ...
  flex-direction: column;
`;
type FetchDataProps = InputProps &
  ButtonProps & {
    message: string;
  };
...
const StatusText: React.FC<StatusProps> = ({ status, count, id }) => (
  <StyledStatusContainer>
    <Typography variant="h6">コネクションのステータス：{status}、</Typography>
    <StyledResponseWrapper>
      <ul>
        <li>
...
const FailText: React.FC = ({ children }) => (
  <Typography variant="h6" color="error">
    現在、{children}
    回コネクションの確立に失敗しています。３回失敗するとコネクションが切断されます。
  </Typography>
);
...
const StyledWebScoketRowContainer = styled.div`
  ...
  padding: 10px 0;
`;
type WebSocketDataProps = ButtonProps &
  StatusProps & {
    message: string;
    failCount: number;
