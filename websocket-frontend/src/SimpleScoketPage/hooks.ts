...
export const useFetchAndWebScoket = () => {
  ...
  useEffect(() => {
    ...
    socket.current.on('connect_error', (error: Error) => {
      ...
      let failCountPrev = 0;
      setFailConnectCount((prevCount) => {
        const nextCount = prevCount + 1;
        failCountPrev = nextCount;
        return failCountPrev > 3 ? 0 : nextCount;
      });
      const id = socket.current?.id || '';
      setConnectionId(id);
      failCountPrev > 3 && socket.current?.disconnect();
    });
    // コネクション切断時
  ...
  const onClickFetch = async (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const response: AxiosResponse<ApiResponse> = await axios.get(
      `http://localhost:${REQUEST_URL.api}/simple`,
      {
        params: {
          message: inputValue,
        },
